...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
             ...
             * 注意：这点很重要，Netty 线程模型的高性能取决于对于当前执行的Thread 的身份的确定。
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                // 如果在pipeline的addLast方法添加handler，callHandlerCallbackLater 方法会根据added属性包装成一个task
                // 此时在invokeHandlerAddedInNeeded方法中执行封装好的task
                // add 任务和 removed 任务的不同在于，
                // 1、add 任务是pipeline 初始化之后调用的任务（通过 Channel 的handlerAdded 方法），
                // 2、removed 是pipeline 结束后执行（通过 Handler 的 handlerRemoved 方法）
                pipeline.invokeHandlerAddedIfNeeded();

                // 通知 promise 已经成功了，你可以执行监听器的方法了，
                // 而这里的监听器则是我们的 dobind 方法中设置的
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            try {
                // 执行成功后，执行通道的fireChannelActive方法告诉所有的handler，已经成功绑定
                doBind(localAddress);
            } catch (Throwable t) {
                safeSetFailure(promise, t);
                closeIfClosed();
